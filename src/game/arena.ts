...
export function mountArena(onComplete: (result: ArenaResult) => void) {
  ...
  const enterArena = () => {
    if (!active || !connected) { message.textContent = "WAIT FOR A RIVAL TO JOIN THE ROOM"; return; }
    if (document.pointerLockElement === renderer.domElement) return;
    const request = renderer.domElement.requestPointerLock();
    if (request) void request.catch(() => { message.textContent = "BROWSER BLOCKED MOUSE LOCK. CLICK THE ARENA AGAIN."; });
  };
  ...
  lockButton.addEventListener("click", enterArena); renderer.domElement.addEventListener("click", () => { if (document.pointerLockElement !== renderer.domElement) enterArena(); else shoot(); });
