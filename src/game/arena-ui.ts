...
export function arenaView(online: boolean, config: ArenaConfig) {
  return `<section class="arena-game" id="arena-game">
    ...
    <div class="arena-menu" id="arena-menu">
      ...
      <div><p class="eyebrow">ONLINE</p><h2>PRIVATE 1V1</h2><p class="arena-muted">Private rooms use Iron Yard so both clients retain the existing unsynchronized room protocol.</p>${online ? `<button id="arena-create" class="outline">CREATE 1V1</button><label>ROOM <input id="arena-room-code" maxlength="6" /></label><button id="arena-join" class="outline">JOIN ROOM</button>` : `<p class="arena-muted">Set VITE_ARENA_SERVER_URL to enable private matches.</p>`}<button id="arena-return" class="text-button">RETURN TO SITE</button></div>
    </div>
    <div class="arena-frame" aria-label="Iron Yard game view">
      ...
      <div class="arena-action-bar" aria-label="Game actions"><button data-arena-panel="buy">BUY WEAPONS <kbd>B</kbd></button><button data-arena-panel="loadout">CHANGE WEAPON</button><button data-arena-panel="settings">SETTINGS</button><button data-arena-panel="pause">PAUSE GAME</button><button id="arena-fullscreen" aria-pressed="false">FULL SCREEN <kbd>F</kbd></button></div>
