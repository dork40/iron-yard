...
export class FpsPlayer {
  ...
  update(dt: number, input: DesktopInput) {
    ...
    const forward = Number(input.keys.has(input.bindings.forward)) - Number(input.keys.has(input.bindings.back)); const strafe = Number(input.keys.has(input.bindings.right)) - Number(input.keys.has(input.bindings.left));
