...
function gameView() {
  ...
  if (round.mode === "rock-paper-scissors") return rpsView(round.playerChoice, round.opponentChoice, round.result, `ROCK PAPER SCISSORS Â· ${aiDifficulty.toUpperCase()}`, [seriesPlayerWins, seriesOpponentWins], round.decisionEndsAt, round.number, seriesPlayerWins === 3 || seriesOpponentWins === 3);
  if (round.mode === "trail-trace") {
    ...
    return traceView(round.pathSeed, `TRAIL TRACE Â· ${aiDifficulty.toUpperCase()} Â· ROUND ${String(round.number || 1).padStart(2, "0")}`, prompt, result?.outcome === "win" ? "YOU WIN" : result ? "ASH TAKES IT" : undefined, Boolean(result) || round.phase === "menu", round.phase === "menu" || round.phase === "result");
  }
  if (round.mode === "bottle-shot") {
    const result = round.result;
    return bottleShotView(round.targetSeed, round.startAt, round.endAt, `BOTTLE SHOT Â· ${aiDifficulty.toUpperCase()} Â· ROUND ${String(round.number || 1).padStart(2, "0")}`, round.playerScore ?? bottleScoreTotal, result?.opponentReactionMs, result?.outcome === "win" ? "YOU WIN" : result ? "ASH TAKES IT" : undefined);
  }
  const result = round.result;
...
function multiplayerGameView() {
  ...
  if (activeMode === "rock-paper-scissors") {
    const title = `${room.mode === "showdown-series" ? "SHOWDOWN SERIES" : "LIVE ROCK PAPER SCISSORS"} Â· ROOM ${room.code}`;
  ...
  if (activeMode === "bottle-shot") {
    ...
    return bottleShotView(shared.targetSeed!, Date.parse(shared.startAt), Date.parse(shared.endAt!), `LIVE BOTTLE SHOT Â· ROOM ${room.code}`, mineScore, opponent?.score, ended ? (shared.winner === "tie" ? "TIE" : won ? "YOU WIN" : "YOU LOSE") : undefined, Boolean(mine));
  }
  ...
  return layout(`<section class="duel" data-phase="${waiting ? "waiting" : activeMode === "word-duel" ? "word" : "draw"}" data-result="${ended ? (won ? "win" : "loss") : ""}"><div class="duel-sky"><div class="duel-sun"></div><div class="cloud cloud-one"></div><div class="cloud cloud-two"></div></div><div class="horizon"></div><div class="street"></div><div class="opponent" aria-hidden="true"><span class="hat"></span><span class="head"></span><span class="torso"></span><span class="arm"></span></div><div class="gunslinger" aria-hidden="true"><span class="player-hat"></span><span class="player-body"></span><span class="hand"><i class="revolver"><b></b></i></span><span class="holster"></span><span class="flash"></span></div><div class="duel-panel"><p class="eyebrow">LIVE ${activeMode === "word-duel" ? "WORD DUEL" : "ORIGINAL QUICK DRAW"} Â· ROOM ${room.code}</p><h1>${label}</h1><p class="duel-prompt" aria-live="assertive">${prompt}</p>${resultRows}<div class="duel-controls">${action}${fullscreenButton()}</div></div></section><section class="scoreboard"><div><span>SERIES</span><b>${room.mode === "showdown-series" ? `${shared.seriesHostWins ?? 0} - ${shared.seriesGuestWins ?? 0}` : ended ? (won ? "WIN" : "LOSS") : "LIVE"}</b></div><div><span>YOUR REACTION</span><b>${mine?.falseStart ? "EARLY" : mine ? `${mine.reactionMs} MS` : "--"}</b></div><div><span>RIVAL REACTION</span><b>${opponent?.falseStart ? "EARLY" : opponent ? `${opponent.reactionMs} MS` : "--"}</b></div><div><span>ROOM</span><b>${room.code}</b></div></section>`);
}
...
function resultPanel() {
  ...
  return `<section class="result-panel" data-outcome="${outcome}"><p class="eyebrow">ROUND REPORT Â· ${modeName}</p><h2>${announcement}</h2><p>${shared ? (outcome === "tie" ? "Neither side takes this round." : outcome === "win" ? "You take the round." : "Your rival takes the round.") : result?.message}</p><div class="result-metrics"><span>YOU <b>${mineScore === undefined ? "--" : `${mineScore}${typeof mineScore === "number" && (round.mode === "original-quick-draw" || round.mode === "word-duel" || round.mode === "ghost-challenge") ? " MS" : ""}`}</b></span><span>RIVAL <b>${rivalScore === undefined ? "--" : rivalScore}</b></span><span>STREAK <b>${profile.winStreak}</b></span><span>PROFILE <b>${profile.title}</b></span></div><div class="duel-actions">${next}${returnAction}</div></section>`;
}
...
function render() {
  ...
  if (page === "mode-select") {
    ...
    root.querySelector(".mode-cards")?.insertAdjacentHTML("beforeend", `<article class="ghost-card"${mode === "ghost-challenge" ? " data-selected=\"true\"" : ""}><p class="eyebrow">MODE 06 Â· AI ONLY</p><h2>Beat Your Best</h2><p>Ghost Challenge races your saved personal-best reaction. No record yet? Beat the clearly marked ${ghostStarterTargetMs} MS starter target to set one.</p><button class="outline" data-mode="ghost-challenge">${mode === "ghost-challenge" ? "SELECTED" : "CHALLENGE YOUR GHOST"}</button></article>`);
    root.querySelector(".mode-cards")?.insertAdjacentHTML("beforeend", `<article class="series-card"${mode === "showdown-series" ? " data-selected=\"true\"" : ""}><p class="eyebrow">SERIES Â· FIRST TO THREE</p><h2>Showdown Series</h2><p>A cinematic five-test gauntlet. Each round reveals a new random direct duel; first to three wins takes the street.</p><button class="outline" data-mode="showdown-series">${mode === "showdown-series" ? "SELECTED" : "START SHOWDOWN SERIES"}</button></article>`);
    root.querySelector(".mode-cards")?.insertAdjacentHTML("beforeend", `<article class="arena-card"${mode === "arena-skirmish" ? " data-selected=\"true\"" : ""}><p class="eyebrow">MODE 08 Â· LIVE 1V1</p><h2>Iron Yard</h2><p>A compact 1v1 arena skirmish. Create or join a room, then move, aim, reload, and outshoot a real rival.</p><button class="outline" data-mode="arena-skirmish">${mode === "arena-skirmish" ? "SELECTED" : "ENTER IRON YARD"}</button></article>`);
