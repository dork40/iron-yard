...
Clients send bounded `{ "type": "state", "x", "z", "yaw", "pitch" }` updates at approximately 15 Hz and `{ "type": "shot", "loadout", "yaw", "pitch" }`. Zod validates every message. The server owns accepted player positions, fire-rate limits, aim-ray hit testing, damage, health, respawns, and disconnect broadcasts. Room state is intentionally volatile: a deployment restart ends active matches. This is a casual protocol, not cheat-proof matchmaking. The browser uses Three.js/WebGL and generates its world materials locally.
