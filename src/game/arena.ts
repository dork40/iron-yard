...
export function arenaView() {
  ...
  return `<section class="arena-game"><div class="arena-heading"><p class="eyebrow">LIVE 1V1 ARENA Â· REAL-TIME 3D</p><h1>IRON YARD</h1><p>A close-quarters duel in a textured WebGL arena. Create a room or join a friend.</p></div><div class="arena-lobby"><p id="arena-connection" aria-live="polite">${configured ? "CREATE A ROOM OR JOIN A FRIEND" : "ARENA SERVER URL REQUIRED"}</p>${configured ? `<div><button id="arena-create" class="primary">CREATE ROOM</button><label>ROOM CODE <input id="arena-room-code" maxlength="6" autocomplete="off" /></label><button id="arena-join" class="outline">JOIN ROOM</button></div>` : `<p>Add <code>VITE_ARENA_SERVER_URL</code> to the Vite build environment and redeploy.</p>`}</div><div class="arena-loadout" aria-label="Choose loadout"><button class="outline" data-arena-loadout="sidearm" aria-pressed="true"><b>RANGER SIDEARM</b><span>6 rounds Â· heavy hit</span></button><button class="outline" data-arena-loadout="carbine" aria-pressed="false"><b>BRUSH CARBINE</b><span>12 rounds Â· rapid fire</span></button></div><div class="arena-frame"><div id="arena-canvas" aria-label="Iron Yard first-person 3D arena"></div><button id="arena-lock" class="arena-lock" type="button">CLICK TO ENTER ARENA</button><div class="arena-reticle" aria-hidden="true"><i></i><b></b></div><div class="arena-status"><span id="arena-health">ARMOR 100</span><span id="arena-ammo">6 / 24</span><span id="arena-rival">RIVAL --</span></div><div id="arena-message" class="arena-message">JOIN A ROOM TO ENGAGE</div></div><div class="arena-controls"><button id="arena-fire" class="primary" disabled>FIRE</button><button id="arena-reload" class="outline" disabled>RELOAD</button></div><p class="trace-hint">CLICK ARENA TO LOCK MOUSE Â· WASD MOVE Â· MOUSE AIM Â· CLICK OR FIRE TO SHOOT Â· R RELOAD Â· ESC RELEASES MOUSE</p><p class="arena-local-note">REAL-TIME 1V1: server-validated rooms, player states, and hits.</p></section>`;
}
