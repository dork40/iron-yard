...
export type FpsSettings = { sensitivity: number; fov: number; crosshair: CrosshairSettings; graphics: GraphicsSettings };
...
export const defaultSettings: FpsSettings = { sensitivity: 0.0022, fov: 78, crosshair: { color: "#f3d78b", size: 9, gap: 5, thickness: 2, dot: true, outline: true }, graphics: { quality: "high", pixelRatio: 1.75, shadows: true } };
...
export function loadSettings(): FpsSettings { try { const saved = JSON.parse(localStorage.getItem(key) ?? "{}") as Partial<FpsSettings>; return { ...defaultSettings, ...saved, crosshair: { ...defaultSettings.crosshair, ...saved.crosshair }, graphics: { ...defaultSettings.graphics, ...saved.graphics } }; } catch { return structuredClone(defaultSettings); } }
